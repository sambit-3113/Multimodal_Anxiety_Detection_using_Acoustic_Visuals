...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        # ===== SAVE BEST MODEL (AUTO NAMED IN weights/) =====
        if val_acc > best_val_acc:
            ...
            save_path = f"weights/{model_name}_{dataset_name}.pt"               ## changed here to save the dataset name as well in the weights folder
    ...
    artifact = wandb.Artifact("best_model", type="model")                   #........ changed till

    dataset_name = os.path.basename(args.data_dir)
    artifact_path = f"weights/{args.model}_{dataset_name}.pt"

    if os.path.exists(artifact_path):
        artifact.add_file(artifact_path)
        print(f"[INFO] Artifact uploaded: {artifact_path}")
    else:
        print(f"[WARNING] Model file not found: {artifact_path}")           #.........till this part 
   ...
   # ===== LOAD BEST MODEL FROM weights/ =====
    dataset_name = os.path.basename(args.data_dir)
    load_path = f"weights/{args.model}_{dataset_name}.pt"                      #changed

    net.load_state_dict(
        torch.load(load_path, map_location=args.device[0])
    )

    print(f"[INFO] Loaded best model from: {load_path}")            #changed here
